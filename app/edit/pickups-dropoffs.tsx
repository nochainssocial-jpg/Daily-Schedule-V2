...
export default function PickupsDropoffsScreen() {
  ...
  // Dropoff labels should only show the participant name once.
  const getDropoffLabel = (p: { id: ID; name: string }): string => {
    ...
    const location = String(options[index] ?? options[0]).trim();
    const participantPrefix = `${String(p.name ?? '').trim()} →`;

    if (
      participantPrefix !== ' →' &&
      location.toLowerCase().startsWith(participantPrefix.toLowerCase())
    ) {
      return location;
    }
  ...
  const isAlternativeDropoff = (p: { id: ID; name: string }): boolean => {
    const options = getDropoffOptions(p);
    ...
    const destination =
      arrowIndex >= 0 ? location.slice(arrowIndex + 1).trim() : location;

    return !/^home(?:\s|\(|$)/i.test(destination);
  };
                          ...
                          >
                            <Text
                              style={[
                                styles.chipTxt,
                                selected && styles.chipTxtSel,
                                isAlternativeDropoff(p as any) &&
                                  styles.dropoffAltLocationTxt,
                              ]}
                            >
