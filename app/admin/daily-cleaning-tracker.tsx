...
import {
  ...
  loadCleaningAssignmentsReport,
} from '@/lib/admin-report-engine';

const DEFAULT_WEEK_OFFSET = 0;
const PAGE_TITLE = 'Cleaning Assignments Tracker';
const PAGE_SUBTITLE = 'Current-week fairness view using the latest saved cleaning duties for each working day.';
const HEADER_TITLE = 'Daily Cleaning Assignment Tracker';
...
export default function AdminWeeklyReportScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const next = await loadCleaningAssignmentsReport(weekOffset);
