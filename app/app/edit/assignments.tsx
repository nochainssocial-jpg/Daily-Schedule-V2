...
export default function EditAssignmentsScreen() {
  ...
  const assignTrainingShadow = (traineeId: ID) => {
    ...
    // Training links are a display/relationship marker only. Never move or
    // clear participant assignments when a trainee is selected. To avoid
    // hiding an independently assigned staff member (and therefore their
    // participants), require their assignments to be moved first.
    ...
    const previousTraineeId = nextShadows[mentorId]
      ...
      : null;
    const displacedTraineeIds = new Set<ID>();

    // A trainee can shadow only one mentor, and a mentor cannot simultaneously
    // be used as another staff member's trainee. This also cleanly reverses an
    // accidentally inverted link such as Gilda -> Jessica into Jessica -> Gilda.
    Object.entries(nextShadows).forEach(([existingMentorId, existingTraineeId]) => {
      if (
        ...
        String(existingTraineeId || '') === mentorId
      ) {
        if (existingTraineeId) {
          displacedTraineeIds.add(String(existingTraineeId));
        }
    ...
    // The person being shadowed is the mentor and must remain an independent
    // assignment row, even if a stale/reversed link previously marked them as
    // training.
    nextTraining.delete(mentorId);

    const linkedTraineeIds = new Set<ID>(
      Object.values(nextShadows).filter(Boolean).map((id) => String(id)),
    );

    if (
      previousTraineeId &&
      previousTraineeId !== selectedTraineeId &&
      !linkedTraineeIds.has(previousTraineeId)
    ) {
      nextTraining.delete(previousTraineeId);
    }

    displacedTraineeIds.forEach((id) => {
      if (id !== selectedTraineeId && !linkedTraineeIds.has(id)) {
        nextTraining.delete(id);
      }
    });
