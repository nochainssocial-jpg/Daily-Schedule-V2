...
// For a given slot, is a particular staff member offsite due to outing at that time?
function isStaffOffsiteForSlot(slot: any, staffId: string, outingGroup: any): boolean {
  if (!outingGroup) return false;

  const outingStaffIds = new Set(
    ((outingGroup.staffIds ?? []) as (string | number)[]).map((id) => String(id)),
  );
  if (!outingStaffIds.has(String(staffId))) return false;

  const outingWindow = getOutingWindowMinutes(outingGroup);
  // No valid window = treat as all-day offsite
...
function buildAutoAssignments(
  ...
  getActiveStaffForSlot?: (slot: any) => any[],
  ...
  (timeSlots || []).forEach((slot, index) => {
    ...
    const baseWorking: any[] = getActiveStaffForSlot ? (getActiveStaffForSlot(slot) || []) : (working || []);
    roomOrder.forEach((col) => {
      let candidates = baseWorking.filter((s) => !thisSlotStaff.has(s.id));
...
const activeWorkingForSlot = (slot: any): any[] => {
  // Exclude staff who are offsite due to outing during this slot window
  return (onsiteWorking || []).filter((s: any) => {
    const id = String(s?.id ?? '');
    if (!id) return false;
    return !isStaffOffsiteForSlot(slot, id, outingGroup);
  });
};
  useEffect(() => {
    // 🔥 Auto-build using *onsite* working staff only
    if (!hasFrontRoom && onsiteWorking.length && updateSchedule) {
      const next = buildAutoAssignments(onsiteWorking, TIME_SLOTS, activeRoomsForSlot, activeWorkingForSlot);
      updateSchedule({ floatingAssignments: next });
          ...
          >
            ...
            >
              ...
              {(sortedWorking || []).map((s: any) => {
                const isOffsiteAny = outingStaffSet.has(String(s.id));
                return (
                  <View key={s.id} style={{ opacity: isOffsiteAny ? 0.35 : 1 }}>
                    <Chip
                      label={s.name}
...
{(() => {
  const pickSlot = (TIME_SLOTS || []).find(
    (s: any) => String(s.id) === String(pick?.slotId),
  );
  return (<View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
              {(onsiteWorking || [])
                ...
                .filter((s: any) => {
                  const slot = pickSlot;
                  if (!slot) return true;
                  return !isStaffOffsiteForSlot(slot, String(s.id), outingGroup);
                })
                .map((s: any) => (
