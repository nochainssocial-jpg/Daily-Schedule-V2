...
export type WeeklyReportLoader = (weekOffset: number) => Promise<WeeklyReportResult>;

type Props = {
  defaultWeekOffset: number;
  pageTitle: string;
  pageSubtitle: string;
  loadReport: WeeklyReportLoader;
};
...
export default function WeeklyReportScreen({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const next = await loadReport(weekOffset);
        ...
        <View style={styles.card}>
          <View style={styles.headerRow}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.description}>{pageSubtitle}</Text>
            </View>
