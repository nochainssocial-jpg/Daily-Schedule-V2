...
import {
  ...
  shouldAutoResetPropertySupport,
...
function restoreAssignmentsForCards(
  savedAssignments: PropertySupportAssignment[],
  activeLocations: PropertyLocation[],
) {
  return restorePropertySupportAssignmentSlots(
    ...
    // Compatibility with Participant Support entries previously stored using
    // a property UUID instead of the user-entered location text.
...
export default function PropertySupportScreen() {
  ...
  useEffect(() => {
    ...
    const refreshAtDailyReset = async () => {
      if (
        resetHandled ||
        refreshInFlight ||
        !shouldAutoResetPropertySupport(todayKey)
      ) {
        return;
      }
      ...
      try {
        ...
        const activeLocations = result.locations.filter(
        ...
        <View style={styles.headerCard}>
          ...
          <View style={styles.headerTextBlock}>
            ...
            <Text style={styles.subtitle}>
              Assign staff to two property support visits and two participant support visits for today. All four tiles clear automatically at 5:00pm.
            </Text>
          </View>
        </View>
